	...
	return (
		N(() => {
			const t = s.page.subscribe(() => {
				c &&
					(r(5, (u = !0)),
					r(6, (h = document.title || "untitled page")));
			});
			return r(4, (c = !0)), t;
		}),
		(t.$$set = (t) => {
...
const M = {},
	...
	Y = [
		() =>
			X(
				() => import("./pages/__layout.svelte-44e0ab67.js"),
				[
					"pages/__layout.svelte-44e0ab67.js",
					"assets/pages/__layout.svelte-d93fe2c8.css",
		...
		() =>
			X(
				() => import("./error.svelte-bc06253e.js"),
				["error.svelte-bc06253e.js", "chunks/vendor-4ee6881a.js"]
			),
		() =>
			X(
				() => import("./pages/index.svelte-53213092.js"),
				[
					"pages/index.svelte-53213092.js",
					"assets/pages/index.svelte-a17562fa.css",
					"chunks/vendor-4ee6881a.js",
					"chunks/notifications-112ca568.js",
				]
			),
	...
	init_listeners() {
		...
		addEventListener("touchstart", s),
			...
			addEventListener("click", (t) => {
				...
				if (e.hasAttribute("download") || (s && s.includes("external")))
					return;
				if (e instanceof SVGAElement ? e.target.baseVal : e.target)
					return;
	...
	if (o && o.textContent) {
		const t = JSON.parse(o.textContent),
			...
			r = ((t, e) => {
				...
				if (null != t && s)
					for (var o of s(t))
						e.indexOf(o) < 0 && a.call(t, o) && (r[o] = t[o]);
				return r;
			})(t, ["body"]);
...
class at {
	...
	async start({ status: t, error: e, nodes: r, page: s }) {
		...
		try {
			for (let a = 0; a < r.length; a += 1) {
				...
				if ((n.push(u), u && u.loaded))
					if (u.loaded.error) {
						...
						u.loaded.context && (l = i(i({}, l), u.loaded.context));
			}
			a = o
			...
			a = await this._load_error({
				...
				error:
					((c = u),
					c instanceof Error ? c : new Error(JSON.stringify(c))),
				path: s.path,
				query: s.query,
			});
		}
		...
		if ((this.invalid.clear(), n.redirect)) {
			if (!(e.length > 10 || e.includes(t.path)))
				return void (this.router
					? this.router.goto(n.redirect, { replaceState: !0 }, [
							...e,
							t.path,
					  ])
		...
		return (
			...
			this.invalidating ||
				(this.invalidating = Promise.resolve().then(async () => {
					const t =
						this.router &&
						this.router.parse(new URL(location.href));
					t && (await this.update(t, [], !0)),
						(this.invalidating = null);
				})),
			this.invalidating
		);
	...
	async _get_navigation_result(t, e) {
		...
		for (let r = 0; r < t.routes.length; r += 1) {
			const s = t.routes[r];
			if (1 === s.length)
				return { reload: !0, props: {}, state: this.current };
			let n = r + 1;
	...
	async _get_navigation_result_from_branch({ page: t, branch: e }) {
		...
		if (a) {
			...
			let r = !1;
			const n = () => {
					this.cache.get(e) === s && this.cache.delete(e),
						i(),
						clearTimeout(o);
				},
		...
		if (r.load) {
				...
				u = {
					...
					fetch(t, e) {
						const r = "string" == typeof t ? t : t.url,
							{ href: n } = new URL(
								r,
								new URL(s.path, document.baseURI)
							);
						return (
							a.uses.dependencies.push(n),
							c ? fetch(t, e) : nt(t, e)
						);
					},
				};
			...
			(a.loaded = (function (t) {
				...
				if (t.error || e) {
					const r = t.status;
					if (!t.error && e)
						return { status: r || 500, error: new Error() };
					const s =
			...
			try {
				...
				if (
					...
					if (t && t.loaded) {
						if (
							(t.loaded.error &&
								((y = t.loaded.status), (p = t.loaded.error)),
							t.loaded.redirect)
						)
							return {
...
async function ot({
	...
	const l = new at({ Root: J, fallback: H, target: e, session: r, host: s }),
		c = n
			? new rt({
					base: t.base,
					routes: F,
